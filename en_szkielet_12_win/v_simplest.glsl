...
void main(void) {
    vec4 lp = vec4(0, 4, -8, 1); //light position, world space
    vec4 lp2 = vec4(5, 10, 40, 1);//light2 position, world space
    l = normalize(V * lp - V * M * vertex); //vector towards the light in eye space
    l2 = normalize(V * lp2 - V * M * vertex);//vector towards the light2 in eye space
